import sys
from PyQt5.QtWidgets import *   # 위젯관련 함수
from PyQt5.QtCore import QTimer # 시간관련 함수 사용
import pyautogui

class MouseClick(QWidget):
    ...
    def start_btn_click(self):
        ...
        self.y = 500                # y축 좌표
        self.delay = 3              # 지원(초)
        self.runtime_number = 3     # 반복 횟수

        self.timer.start(self.delay * 1000)             # timer 시작, self.delay만큼 시간 지연 후 종료 그리고 다시 기동
    ...
    def mouse_click(self):
        ...
        if self.num_click == self.runtime_number:       # 클릭 횟수가 self.runtime_number와 같을때 멈춰라
            self.timer.stop()
...
if __name__ == '__main__':          # import된 것들을 실행시키지 않고 __main__에서 실행하는 것만 실행 시킨다.
    ...
    ex = MouseClick()                # ex에 MouseClick()을 상속시킨다.
